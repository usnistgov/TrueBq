...
void PhysListEmStandardNR::ConstructProcess()
{
  ...
  nucr->SetRecoilCutoff(.1 * eV); // set to 0.1 eV to get recoils below 196 eV (reference frame issue?)
  nucr->SetMFPScaling(.04); // for short range set < 1
  nucr->SetPhysicsCutoff(.1 * eV);
  // G4cout << "ScreeningKey = " << nucr->GetScreeningKey() << G4endl;;

  // more options at the end of this file and in PhysicsList (cuts)
...
//      eIoni->SetMinKinEnergy(10 * eV); // RPF - did not work to push production below 990 eV in Si. Maybe eMutlipleScattering? or Coulomb?
      // bremsstrahlung
    ...
    //  ionIoni->SetMinKinEnergy(10 *eV); // RPF - did not push e- production below 990 eV (for visualization). note ionIoni is process producing e-s
  ...
  //physics tables
  //
  emOptions.SetMinEnergy(.05*eV);        // I changed this to 1 eV (from 10 eV) to get events down to 200 eV
  emOptions.SetMaxEnergy(10*MeV);      // was 10*TeV
  emOptions.SetDEDXBinning(12*20);      
  emOptions.SetLambdaBinning(12*20); 
  
  //G4ProductionCuts * cuts = new G4ProductionCuts;
  //cuts->SetProductionCut(.1 * um); // general. Includes recoil ions, though can't seem to set them per se. 
  //cuts->SetProductionCut(1 * mm, "e-"); // 
  //cuts->SetProductionCut(1 * mm, "e+"); // 
  //cuts->SetProductionCut(1 * mm, "gamma"); // 

  //emOptions.SetApplyCuts(cuts);
